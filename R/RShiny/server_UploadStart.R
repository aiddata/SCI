...
#Function takes histogram setting, reclassifies rows accordingly, and provides a very quick summary of input
#covariates and outcomes data.
output$DescTreatment <- renderUI({
  if (!is.null(input$data_sel) & !is.null(input$TrtBin)){
  inData <- uploadShpfile()
  inData["TrtBin"] = 0
  #inData[""]
...
output$DescSelAnc<- renderUI({
  if (!is.null(uploadShpfile())){
    shpFile <- uploadShpfile()
    selectInput("anc_sel","Select Ancillary Data", names(shpFile@data),multiple=TRUE)       
  }
})

output$DescSelOut<- renderUI({
  if (!is.null(uploadShpfile())){
    shpFile <- uploadShpfile()   
    selectInput("outcome_sel","Select Outcome Data", names(shpFile@data),multiple=TRUE)  
  }
})
