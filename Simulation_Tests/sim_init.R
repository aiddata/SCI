#Wrapper for simulation tests of SpatialCausal package.
 #Generating Random Spatial Fields

source('Dependencies/dep.R', chdir=T)
source('Tools/descriptives.R', chdir=T)
source('Tools/SpatialCausalPSM.R', chdir=T)
source('functions.R', chdir=T)

#Number of Iterations
its = 100

#How big the field will be
x <- seq(1, 10, 1)

#Equations to fit
PSM_eq = "TrtBin ~ ControlA"
Final_eq = "y ~ TrtBin + ControlA"

#-----------------------------------------
iteration = 1
beta_df <- data.frame(matrix(ncol=7, nrow=its))
colnames(beta_df)[1] <- "BdifBhat"
colnames(beta_df)[2] <- "YMorans"
colnames(beta_df)[3] <- "TMorans"
colnames(beta_df)[4] <- "XMorans"
colnames(beta_df)[5] <- "EAMorans"
colnames(beta_df)[6] <- "EBMorans"
colnames(beta_df)[7] <- "Model_Type"

while (iteration <= its)
{
  f.SPDF <- SpatialCausalSim_DGP(fld_size = x)
  
  f.NB = poly2nb(f.SPDF)
  f.W = nb2listw(f.NB, style='W')

  beta_df["XMorans"] = moran.test(f.SPDF@data$ControlA, f.W)$estimate[[1]]
  beta_df["YMorans"] = moran.test(f.SPDF@data$y, f.W)$estimate[[1]]
  beta_df["TMorans"] = moran.test(f.SPDF@data$TrtBin, f.W)$estimate[[1]]
  beta_df["EAMorans"] = moran.test(f.SPDF@data$RandomFieldA, f.W)$estimate[[1]]
  beta_df["EBMorans"] = moran.test(f.SPDF@data$RandomFieldB, f.W)$estimate[[1]]
  
  
  
  psm_Res <- SpatialCausalPSM(dta = f.SPDF, mtd = "logit", PSM_eq, drop="overlap", visual="FALE")
  psm_Pairs <- SpatialCausalDist(dta = psm_Res, mtd = "fastNN", vars = PSM_eq, ids = "simIDs", drop_unmatched = TRUE, drop_method = "SD", drop_thresh=.5, visual="FALSE")
  
  iteration = iteration + 1
  
}


